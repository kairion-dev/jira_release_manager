...
class Core {
  ...
  constructor() {
    if (!instance) {
      ...
      // init Jira library
      let configJira = JSON.parse(JSON.stringify(config.get('jira')));
      if (config.has('jira.oauth.consumer_secret')) {
        configJira.oauth.consumer_secret = fs.readFileSync(config.get('jira.oauth.consumer_secret'), "utf8");
      }
      this.jira = new JiraApi(configJira, db);

      // git property for initializing repositories later on
      this.git = {};

      if (!configJira.issuePrefix) {
        throw new Error('Please define jira.issuePrefix in your config');
      }
      this.issuePrefix = configJira.issuePrefix;
        ...
        return db.tags.findAsync({})
          .map((doc) => {
            return Promise.map(doc.tickets,
              (ticket) => {
                if (!ticket.startsWith(this.issuePrefix.quickfix)) {
