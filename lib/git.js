...
class GitHistory {
  ...
  initOpenBranches() {
    var knownTags = {};
    return this.db.tags.removeAsync({ type: 'branch' }) // quick fix to be able to update open branches
      .then(() => this.db.tags.findAsync({ type: 'branch' }))
      .then((docs) => {
  ...
  initTags() {
    ...
    return this.db.tags
      ...
      // Handle 'Next Release'
      .then(() => {
        return this.db.tags.removeAsync({ type: 'release', tag: 'Next Release' })
