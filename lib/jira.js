...
class Jira {
  ...
  constructor(options, db) {
    this.options = {
      ...
      issuePrefix: {
        quickfix: options.issuePrefix.quickfix
      }
    };
    this.db = db || false;
  ...
  fetchIssues(issueKeys, userOptions) {
    ...
    return Promise
      .map(issueKeys, (issueKey) => {
        ...
        if (issueKey.startsWith(this.options.issuePrefix.quickfix)) {
          options.fetchedIssues[issueKey] = {
            key: this.options.issuePrefix.quickfix + ' (' + options.zeroCounter + ')',
            project: this.options.issuePrefixDevelop,
            summary: issueKey.replace(this.options.issuePrefix.quickfix, '').trim(),
            status: 'Done',
            issueType: 'QuickFix',
            assignee: '',
            components: [],
        ...
        return this.jira
          ...
          .then((issue) => {
            var dbDoc = {
              ...
              assignee: issue.fields.assignee.displayName ? issue.fields.assignee.displayName : 'Missing',
              components: [],
              parent: false,
              newCap: false
            };
  ...
  /**
   * Generate a parent child tree, which means linking the tickets to each other.
   * @param {Object} tickets
   *   Array of issues
   * @return {Promise}
   *   Array of resulting issues
   */
  linkChildren(tickets) {
    var tmp = {}, children = {}, resultTickets = [], toDelete = [];
    ...
    Object.keys(children).forEach((ticket) => {
      if (tmp[ticket]) { // we have to check because ticket could be a child and thus already been deleted
        ...
        children[ticket].forEach((child_id) => {
          ...
          // do not already delete the ticket but collect it because it could be still required as a parent itself
          toDelete.push(child_id);
        });
      }
    });
    // delete top level tickets which are used as a child
    toDelete.forEach((child_id) => {
      delete tmp[child_id];
    });
