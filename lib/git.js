...
class GitHistory {
  ...
  constructor(options, db) {
    ...
    this.issuePrefix = {
      develop: options.issuePrefix.develop,
      quickfix: options.issuePrefix.quickfix,
      regex: new RegExp(options.issuePrefix.regex)
    };
    this.db = db;
  }
    ...
    return this.db.tags.removeAsync({ type: 'branch', repository: this.git_name }, { multi: true }) // quick fix to be able to update open branches
      ...
      .then((references) => {
        ...
        return Promise.filter(references, (ref) => {
          return ref.indexOf(this.featurePrefix) != -1
        })
      })
      .then((branches) => {
        return Promise.map(branches, (tag) => {
          tag = tag.replace(this.featurePrefix, '');
          // ignore already processed tags
          if (knownTags[tag]) {
            return;
          }
          return this.gitLog('develop', this.featurePrefix + tag)
  ...
  */
  storeTag(tag, commits, type, preliminary) {
    ...
    commits.forEach((c) => {
      ...
      let result = message.match(this.issuePrefix.regex);
      if (result && result.length > 1) {
        if (result[1] != this.issuePrefix.quickfix) {
        ...
        if (message.startsWith('Merge')) {
          ...
          tickets.push(this.issuePrefix.quickfix + ' ' + message);
        }
      }
    });
